...
### Top-player rule (Eugene 2026-05-22)

**Правила «верхнего» плеера** на главной (`landing.tsx` currentTrack big player + countries/top-100 panels). Любое изменение плеера должно соответствовать этим стандартам. Зафиксировано после серии итераций 22.05.2026.

**Визуальная архитектура:**

1. **3D Земля вместо emoji 🌍** — `<PlanetIcon size={32}>` из `plays-counter.tsx`:
   - Equirectangular SVG world strip 200×100 units
   - SMIL `animateTransform` 60s/полный оборот (continents)
   - Glossy ocean radial gradient + cyan atmosphere halo
   - Specular highlight + fresnel rim + terminator shadow = 3D sphere look
   - `prefers-reduced-motion` safe

2. **Stats row layout** — `[Скачать][Поделиться] [🌍/24] [эквалайзер] [🎧/7916]`:
   - All центрированы по горизонтали с Share button (h-8 w-8 box каждый)
   - 🌍 и 🎧 — `text-3xl` (30px) emoji/SVG в h-8 box
   - Числа countries/plays — `absolute top-full mt-2 left-1/2 -translate-x-1/2`
   - Эквалайзер 12 bars `flex-1 h-12` `bg-gradient-to-t from-purple-500 via-fuchsia-500 to-cyan-400`
   - `animationPlayState` sync с `isPlayingState`

3. **Same-zone-same-style** (см. одноимённое rule):
   - Все цифры `text-[13px] font-bold tabular-nums`
   - Plays gradient: `from-purple-400 via-violet-300 to-cyan-300`
   - Countries gradient: `from-blue-300 via-cyan-300 to-emerald-200` (одна яркость с plays)
   - Track titles в panels: `font-medium gradient from-purple-300 via-fuchsia-200 to-cyan-300` (звёздный)
   - currentTime/duration в плеере: `text-[13px] font-sans tabular-nums text-muted-foreground`

**Panels (countries 🌍 и top-100 🎧):**

4. **Anchored bubble pattern** — оба над своими кнопками:
   - Width: `w-[280px] max-w-[80vw]` (mobile fixed bottom-sheet style: bottom-20 left-3 right-3)
   - Height: dynamic max-height через DOM measurement до hero h1 «минуту» (не залезает выше)
   - Background: chat-pane style — `bg-background/[0.28] backdrop-blur-md border-2 border-purple-400/40`
   - Shadow: `shadow-2xl shadow-purple-500/20`
   - Header bg: `bg-purple-500/5` + gradient title `from-purple-300 via-fuchsia-300 to-cyan-300`
   - Gap от button: `mb-3` (12px)

5. **flex-col-reverse** — 1-я строка (top по metric) **внизу near button**, последняя **вверху**.

6. **Staggered appearance** — каждая строка `animate-in fade-in slide-in-from-bottom-2 duration-700`:
   - `animationDelay: Math.min(idx, 25) * 200ms` (после 25-го — instant)
   - Эффект «падают сверху на стэк»

7. **Close UX** — 500ms cooldown (`Accidental-tap-protection`), tap-anywhere closes с fade-out `animate-out fade-out duration-200`.

8. **Countries panel content**:
   - Server sort by visits DESC (no Russia priority override)
   - Row order: **English name (gradient)** слева → **flag** справа
   - English names via `Intl.DisplayNames(['en'], {type:'region'})`
   - Count `n` НЕ показывается в этой панели (Босс «цифр нет»)

9. **Top-100 panel content**:
   - Slice 100 треков sorted by plays DESC
   - Row: `[rank] [cover 32px] [title gradient] [plays gradient]`
   - Cover `<img src="/api/cover/<id>.jpg">` w-8 h-8 rounded-lg object-cover с brand fallback bg

**Playlist builder (swipe gestures на rows top-100):**

10. **Swipe-right ≥60px → add to playlistBuilder (зелёная panel)**:
    - Live tint: green intensity `|offset|/200` (max 45%) при swipe
    - Trек **исчезает** из top-100 (filter out по `playlistBuilder.includes`)
    - Появляется в **side panel** правой стороне:
      · `fixed bottom-20 right-3 sm:bottom-6 sm:right-6`
      · `border-2 border-green-400/70 shadow-green-500/30`
      · Track row staggered slide-in-from-right с 80ms delay
      · ▶ button «Запустить плейлист» внизу panel

11. **Swipe-left ≥60px → add to excludedTrackIds (красный)**:
    - Live tint: red intensity такая же formula
    - Трек **исчезает** из top-100 (filter `!excludedTrackIds.includes`)
    - Оставшиеся continue играть, можно swipe-left dale удалять по ходу

12. **10-сек inactivity timer** после последнего add → Музa-prompt над green panel:
    - «Сохраню как супер плейлист?» [Да, сохрани] [Не надо]
    - Save → localStorage `muzaai-super-playlist-{userId|guest}`

13. **Load saved prompt** при mount:
    - Через 3 сек после load — bottom-right bubble «У тебя личный плейлист на N треков. Включить?»
    - [▶ Да, включить] → restore + playTrack(first)
    - [Не сейчас] → dismiss, плейлист continues своим ходом

**Музa-FAB:**

14. **Swipe-right на Музу ≥60px** → `dismiss()` (existing logic 60с REAPPEAR_FIRST / 1ч SECOND)
    - 3-tap по пустой области = wake-up early
    - DoubleClick = also dismiss (existing)

15. **Toast-hints о новых фишках** (8 штук) — Музa проактивно рассказывает:
    - First show через 15 сек после mount
    - Each next через 60 сек, random из unseen
    - Dedup: `localStorage muzaai-seen-features-v1`
    - НЕ показывает если чат с Музой открыт
    - Tone: Музa-female-voice (от лица девушки)

**Big planet (опционально, попалить если нужно):**

16. **3-сек hold на 🌍** → big planet overlay (если возвращаем эту фичу):
    - 320×320 mobile / 480×480 desktop
    - Кольцо флагов стран posetiteley вокруг planet
    - Auto-rotate 60s/full + drag-to-rotate manual override
    - **НЕ использовать 3D `rotateY` + `transformStyle: preserve-3d`** — ломает rendering (был revert'нут)
    - 2D `rotate()` + counter-rotate flags works

**Cover-details-modal:**

17. **Backdrop как .glass-card** основной плеер:
    - `bg: rgba(18,18,22,0.85)` + `backdrop-filter: blur(40px) saturate(180%)`
    - inset top highlight

18. **Cover container** с brand gradient (как main player cover):
    - `bg-gradient-to-br from-purple-500/30 to-blue-500/30`
    - `shadow-lg shadow-purple-500/10`

19. **Container width +15%** от первоначального:
    - md: max-w-[500px] w-[80vw], lg: max-w-[580px] w-[70vw]
    - Cover max-h: 63/69/75/78vh по breakpoints

20. **items-end + pb-4 sm:pb-6** — карточка прижата к низу, фон виден сверху.

**Применяется к:** `landing.tsx` верхняя зона (Hero CTA + большой плеер + panels). НЕ применяется к: dashboard.tsx (там свой PlaysCounter + visitor stats), admin pages.

**Counter auto-update:** все `/api/playlist/stats` + `/api/public/countries-count` — клиентский `setInterval 60_000ms`, никаких F5 / window.reload.

**Эталонные commits** (стабильная редакция 22.05.2026 после revert'ов проблемных):
- `c53af2b` — base после revert'ов (3D Earth + panels stable)
- `1cb8543` — empty marker «🔥 Триумф плеера 22.05.2026»
- `83075b6` — covers в top-100
- `c798d95` — swipe-right add to green panel
- `1e96790` — swipe-left exclude red
- `ce1d833` — save/load super-playlist prompts
- `f80b97e` — Музa swipe-right dismiss
- `ff47d47` — Музa feature hints toasts

**Audit перед любым изменением плеера:** проверить соответствие 20 пунктам выше. Любое отклонение требует обновления этого правила.
