...
### Deploy-target-options rule (Eugene 2026-05-08)
...
### Deploy-via-GitHub rule (Eugene 2026-05-15)

**Любой deploy MuziAi (prod + clone) идёт ИСКЛЮЧИТЕЛЬНО через `git push` в GitHub → auto-deploy подхватывает на VPS. Ручного SSH-deploy (rsync, scp, ssh + pm2 restart) НЕ ДЕЛАЮ.**

Цепочка:
```
local edit → git commit → git push -u origin <branch>
   → GitHub (origin)
      → auto-deploy webhook / cron на VPS
         → git pull + npm ci + npm run build + pm2 restart
            → health-check /api/example/ping
               → rollback из pre-flight backup если health fail
```

Почему так:
1. **Аудит** — каждый deploy виден в git log + GitHub timeline. SSH-deploy невидим.
2. **Rollback гарантирован** — `auto-deploy.sh` снимает pre-flight backup ДО pull. SSH-deploy легко затирает рабочий dist без backup.
3. **Health-check встроен** — auto-deploy.sh проверяет `/api/example/ping`. SSH-deploy этого не делает.
4. **Один source of truth** — GitHub. Прод и clone подтянутся одинаково. SSH-deploy создаёт drift между серверами.
5. **Безопасность секретов** — `.env` не покидает VPS. SSH-deploy провоцирует rsync с локальной машины, где может оказаться неправильный .env.
6. **Eugene видит весь процесс** — git push в terminal + countdown 90 сек + GitHub PR/commit URL = полная прозрачность.

Исключения (SSH-команды РАЗРЕШЕНЫ для):
- Read-only: `pm2 status`, `pm2 logs`, `grep`, `curl health-check`, `awk -F=` для проверки .env (без раскрытия значений).
- Ротация секретов: `sed -i + echo + pm2 restart --update-env` (см. Key rotation pattern).
- Аварийный rollback когда auto-deploy сломан: `git reset --hard <PREV_SHA> + pm2 restart` (Eugene даёт явное «да»).
- Backup-операции: `cp data.db /var/backups/...` (read-only к данным).

НЕЛЬЗЯ через SSH (только git push):
- ❌ Редактирование `.ts`/`.tsx`/`.js`/`.html` файлов на VPS напрямую (теряется в следующем git pull)
- ❌ `rsync` локального dist на VPS
- ❌ `scp` отдельных файлов в `apps/neurohub/`
- ❌ `pm2 reload ecosystem.config.js` с изменённой конфигой не из git
- ❌ Изменения миграций / schema через SQL-консоль (только через `storage.ts` auto-migrate)

Если auto-deploy сломан и нужен hot-fix — сначала пытаюсь починить auto-deploy (root cause), и только если этого не получается за ≤15 минут — ручной deploy с явным «да» от Eugene + record в `docs/strategy/PITFALLS.md`.

Применяется к: всем изменениям кода / конфига / документации.
