...
function readSmtpConfig(): SmtpConfig | null {
  ...
  return {
    ...
    user,
    pass,
    from: process.env.SMTP_FROM || `MuziAI <${user}>`,
  };
}

const TEMPLATES: Record<string, { subject: string; body: (vars: any) => string }> = {
  welcome_1: {
    subject: "Добро пожаловать в MuziAI",
    body: (vars) =>
      `Привет!\nСпасибо, что присоединился. Первая генерация — бесплатно.\n\n— Команда MuziAI`,
  },
  welcome_2: {
    subject: "Подсказка: используй шаблоны",
    body: () =>
      `Если не знаешь, с чего начать — попробуй шаблон ('Свадебная песня', 'Гимн компании' и др.).\nКаждый — это готовая структура и стиль.`,
  },
  welcome_3: {
    subject: "Создай первую песню сегодня",
    body: () =>
      `Ты ещё не сделал первую генерацию. Это бесплатно — заходи и попробуй.\n\nclone.muziai.ru`,
  },
  churn_back: {
    subject: "Скучаем по тебе",
    body: (vars) =>
      `Прошло уже больше месяца с твоей последней генерации.\nПосмотри, что нового на платформе.\n\nclone.muziai.ru`,
  },
};
