...
4. **Чек-онлайн (54-ФЗ)** — Robokassa поддерживает фискализацию через `Receipt` параметр. Нужно ли его передавать (зависит от налогового статуса магазина)? Сейчас не передаётся.
5. **Multi-currency** — Robokassa может принимать оплату в разных валютах через `IncCurrLabel`. Сейчас все платежи в рублях. Расширение нужно?
