...
**Скорость работы пропорциональна расходу токенов — максимум параллелизма, минимум ожидания.** Босс «Скорость пропорционально токенам правило».

При срочной задаче (дедлайн / «ускорься» / «через 5 мин»):
- Спавнить субагентов ВЕЗДЕ где можно (Subagent-by-default × 10)
- Параллелить независимые правки в одном tool-call message
- НЕ читать файлы целиком — Read с offset/limit/grep
- Краткий ответ Боссу — только status + действие, без объяснений (объяснения в commit message)
- Token-economy ослабляется когда Босс дал «ускорься» — трачу токены свободно

При штатном режиме — Token-economy + Working rhythm rules.

Применяется к: всем задачам где Босс просит ускориться. Связано с: Subagent-by-default rule, Working rhythm rule, Autonomous-execution rule.
