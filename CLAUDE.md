...
### Post-push 110s delay rule (Eugene 2026-05-30)

**После фактического `git push` — выжидаю 110 секунд, потом пишу Боссу что запушил.** Auto-deploy timer на VPS подхватывает за ~60-90 сек, ещё ~10-20 сек на build + pm2 restart + health-check. К моменту ответа Боссу — код УЖЕ на проде, не «через минуту», а сейчас.

Реализация: после `git push` → `sleep 110` (Bash run_in_background или прямой sleep с timeout) → ответ Боссу «запушил <что>».

НЕ применяется к: правкам только CLAUDE.md / docs (на прод не влияют — Боссу не нужно ждать), очевидным ошибкам (TS-error в push fail — сразу пишу). Связано с: Deploy countdown rule, Prod-auto-deploy + versioned backup rule.
