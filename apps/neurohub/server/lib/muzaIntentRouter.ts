...
// Eugene 2026-05-23 Босс «найди и запусти трек по попаданию букв» — расширено:
// помимо «найди трек/песн» теперь ловит просто «найди X», «поставь X», «включи X»,
// «у тебя есть про X» (substring-поиск через find_public_track).
const PLAYER_SEARCH = /(найди (?:трек|песн|музык|в моих|мой|моё|моей)|^найди\s+\S+|^поставь\s+\S+|^включи\s+\S+|трек\s+про|песн[яею]\s+про|есть\s+(?:что|какой|какая|трек|песн)\s*(?:то|нибудь|у тебя)?\s*про|поищи\s+(?:трек|песн))/i;
