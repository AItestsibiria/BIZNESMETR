...
### Work-limit-warning rule (Eugene 2026-05-15)

**Предупреждаю Eugene о приближении лимита работы за 30 минут до окончания.** Под «лимитом работы» понимается:

- Контекстное окно сессии приближается к компрессии (когда система предупреждает о near-limit)
- Если я веду долгую multi-task session (>4 часов непрерывной работы) — предупреждаю что скоро понадобится передышка / новая сессия
- Если приближается end-of-day / естественная пауза (например 23:00 MSK) — предлагаю зафиксировать триумф-tag и завершить блок

Формат предупреждения:

> ⏰ **Через ~30 мин подойдёт лимит работы.** Что успеть до этого:
> 1. Доделать текущую задачу (если близко к концу)
> 2. Сохранить triumph-tag для recovery point
> 3. Отдельным сообщением — список того что осталось на следующую сессию

Конкретные триггеры:
- Контекст ≥75% от лимита (если я могу это определить)
- Прошло >3 часов непрерывной работы (по timestamps в footer'ах ответов)
- Завершение большого блока (например после 10+ коммитов подряд)

Применяется к: длительным рабочим сессиям с Eugene где есть risk что мы упрёмся в context-limit и потеряем плавность. Не применяется к: коротким QA-сессиям, точечным вопросам.

### Стратег-Критик rule (Eugene 2026-05-16)

**Перед каждым изменением и периодическим аудитом — играю две роли: реализатор + критик.** Не один реализатор без проверки.

Как работает:
1. **Реализую** — пишу код / делаю правку
2. **Критикую** — сам или через subagent (Explore type, read-only) ищу:
   - Скрытые зависимости / cross-references что сломаются
   - Нарушенные логические цепочки (A зависит от B, B изменился)
   - Недоделки (TODO в комментах, частичные миграции)
   - Безопасность frontend + backend (XSS, SQL-injection, CSRF, exposed secrets, weak auth)
   - Регрессии rebrand'а (где осталась старая логика/строка)
   - Последствия для дальнейшего применения (что станет работать иначе через месяц)
3. **Спорю** — критик выдвигает риски, реализатор защищает решение или адаптирует
4. **Итог** — отчёт с таблицей: проблема / серьёзность / варианты решения

Применяется к:
- Большие коммиты (>50 строк или >3 файла)
- Глобальные изменения (rebrand, migration, refactor)
- Изменения в auth / payments / db schema
- Любая просьба «провести ревизию» или «найти проблемы»

Цель: скрупулёзность > скорость. Eugene предпочитает потерянное время на анализ над потерянным временем на rollback.
