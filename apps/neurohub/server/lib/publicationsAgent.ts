...
// «Фишки» MuzaAi для публикаций (Босс «Директор для публикаций учитывает фишки?»).
// Клиент-safe (без имён ИИ-провайдеров — No-AI-providers-in-userland rule). Директор
// ротацией вплетает одну фишку в каждый черновик. Обновлять при появлении новых.
const PRODUCT_HIGHLIGHTS: string[] = [
  "🌍 Живой 3D-глобус Земли: видно в реальном времени, в каких странах слушают Музу — с настоящим Солнцем, Луной и сменой дня и ночи. Картой мира можно поделиться одной кнопкой.",
  "🛰 Над глобусом пролетает МКС, а столицы стран загораются огоньками — красивый «приём сигнала» по всему миру.",
  "🔗 Поделись своей Музой: открой карту мира и отправь друзьям — они увидят живую планету и услышат трек.",
  "🎁 Первый трек — в подарок: расскажи повод, получи готовую песню за пару минут.",
  "🎤 Муза сама подберёт голос, стиль и текст под твой повод — тебе нужно только рассказать историю.",
  "💜 Музу можно слушать прямо на главной — плейлист авторов со всего мира.",
];

function dailyHighlight(channel: PublicationChannel): string {
  // Ротация по дню + каналу, чтобы посты не повторяли одну и ту же фишку.
  const dayIdx = Math.floor(Date.now() / 86400000);
  const chOff = channel.length; // лёгкий сдвиг по каналу
  return PRODUCT_HIGHLIGHTS[(dayIdx + chOff) % PRODUCT_HIGHLIGHTS.length];
}
...
function buildCreativePrompt(channel: PublicationChannel): string {
  return (
    ...
    `Обыграй СЕГОДНЯШНЮЮ фишку платформы (естественно, не дословно):\n` +
    `«${dailyHighlight(channel)}»\n\n` +
    `Жёсткие требования:\n` +
