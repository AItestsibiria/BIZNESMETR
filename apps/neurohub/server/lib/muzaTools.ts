...
const HANDLERS: Record<string, ToolHandler> = {
  ...
  // Eugene 2026-05-17 Босс «техподдержка». Tools для работы Музы с support-ticket'ами.
  // ticket — это запись в agent_handoffs (per No-duplicates rule, переиспользуем).
  ...
  async resolve_ticket({ ticketId, summary }, { userId }) {
    ...
    try {
      ...
      // Запись в admin_audit_log для трассировки auto-resolve Музой.
      try {
        recordAuditEntry({
          adminUserId: null,
          adminEmail: "muza-bot",
