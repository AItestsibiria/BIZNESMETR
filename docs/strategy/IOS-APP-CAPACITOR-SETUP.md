...
Версия: 1.1 (Eugene 2026-05-23) — обновлено после reанимации: PNG-исходники сгенерированы, npm scripts добавлены, brand SVG в репо.
...
Capacitor config + зависимости + brand SVG/PNG ассеты уже в репо. На Mac Босс делает:

```bash
cd apps/neurohub
npm install
npm run build
npx cap add ios       # создаст apps/neurohub/ios/
npm run ios:assets    # сгенерирует все размеры иконок из ios-assets/icon.png + splash.png
npm run ios:build     # build + copy + sync
npm run ios:open      # откроет Xcode
```

Дальше в Xcode: select team → Archive → Submit. Расчёт времени см. ниже в «Estimated time on Mac».

## Что уже готово в репо (статус на 2026-05-23)

- ✅ `apps/neurohub/capacitor.config.ts` — полный конфиг (bundle ID, splash, status bar, preferredContentMode, scheme)
- ✅ `apps/neurohub/package.json` — все `@capacitor/*` deps + npm scripts (`ios:add`, `ios:copy`, `ios:sync`, `ios:open`, `ios:assets`, `ios:build`)
- ✅ `apps/neurohub/ios-assets/icon-source.svg` + `icon.png` 1024×1024
- ✅ `apps/neurohub/ios-assets/splash-source.svg` + `splash.png` 2732×2732
- ✅ `apps/neurohub/ios-assets/README.md` — инструкция + команды
- ✅ `.gitignore` — `ios/App/Pods/`, `ios/App/build/`, `ios/.DS_Store` исключены
- ✅ Все правила (Apple-audio-best-practices, iOS-lock-screen-audio, Persistent-audio-only) уже в `CLAUDE.md`, client code им следует

**Чего ещё нет** (требует Mac + Xcode + Apple Developer account):
- ❌ `apps/neurohub/ios/` directory (генерируется через `npx cap add ios` на Mac)
- ❌ Apple Developer team в Xcode signing
- ❌ Загружены screenshots в App Store Connect
- ❌ Заполнена App Privacy questionnaire
...
### Шаг 5: Сгенерировать иконки + splash из готовых исходников

`apps/neurohub/ios-assets/icon.png` (1024×1024) и `splash.png` (2732×2732) **уже в репо** — сгенерированы из SVG через sharp при reanimации (Eugene 2026-05-23). Если хочется поправить — отредактировать `icon-source.svg` / `splash-source.svg` и пересобрать PNG (команда в `ios-assets/README.md`).
...
#     --iconBackgroundColor "#0a0a17" --splashBackgroundColor "#0a0a17"
```

Это создаст 18+ файлов размеров иконок в `ios/App/App/Assets.xcassets/AppIcon.appiconset/` + splash в `Splash.imageset/`.
...
## Estimated time on Mac (для Босса)

| Шаг | Время | Комментарий |
|---|---|---|
| Скачать Xcode из Mac App Store | 30-60 мин | 15-20 GB, зависит от скорости интернета |
| Установить CocoaPods (`sudo gem install cocoapods`) | 5 мин | разово |
| Apple Developer enrollment | 24-48 часов | подача формы 30 мин, ожидание одобрения от Apple |
| `git clone biznesmetr` + `npm install` | 5-10 мин | первый раз, после — секунды |
| `npm run build && npx cap add ios` | 3-5 мин | Capacitor генерирует ios/ + CocoaPods install |
| `npm run ios:assets` (генерация всех иконок) | 30 сек | если @capacitor/assets установлен |
| `npm run ios:open` + signing в Xcode | 5-10 мин | первый раз — выбор team, certificate setup |
| Build на Simulator (▶️) | 30-60 сек | первый раз — 2-3 мин, дальше — кэш |
| Build на физическом iPhone | 5-10 мин | первый раз требует Developer Mode на iPhone |
| Xcode Archive (Product → Archive) | 5-10 мин | release build с optimisations |
| Upload в App Store Connect | 10-20 мин | автоматически из Organizer |
| Заполнить metadata + screenshots в App Store Connect | 60-90 мин | первый раз медленно (App Privacy questionnaire ~30 мин) |
| **Submit → Apple Review** | **1-7 дней** | первый раз дольше, потом 24-48ч |

**Итого до первого submit:** ~2-3 часа активной работы (если Apple Developer уже одобрен) + 1-7 дней ожидания.

**Итого до App Store live:** ~3-7 дней с момента «у меня готовый Xcode и аккаунт».
