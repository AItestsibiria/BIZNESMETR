...
живой.`;
  ...
  app.post("/api/muza/chat/init", async (req: Request, res: Response) => {
    try {
      ...
      // Eugene 2026-05-14 Босс: первое сообщение тоже получает 3 QR-кнопки.
      // Для не-РФ — отдельные кнопки чтобы юзер мог подтвердить/опровергнуть страну.
      let initialQR: string[];
      if (paired || authUserId) {
        initialQR = ["Продолжим", "Покажи мои треки", "Помоги с новым"];
      } else {
        // Если приветствие включает «Россия приветствует автора из X» —
        // нужны кнопки подтверждения страны.
        const isForeignGreeting = greeting.includes("Россия приветствует автора из");
        initialQR = isForeignGreeting
          ? ["Да, я отсюда", "Я из другой страны", "Расскажи о проекте"]
