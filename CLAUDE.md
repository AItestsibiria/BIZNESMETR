...
2. **MuziAi** (`muziai.ru` на VPS 31.130.148.107) — production. Ручной deploy через GitHub Actions workflow или SSH-скрипт. Только после проверки на clone.
...
3. **При обнаружении leaked секрета** — немедленно прошу Eugene отозвать в провайдере + создать новый + ввести через прямой SSH в Termius (минуя чаты). Замена без отзыва не допустима.
