...
Ниже — пошаговая инструкция, как создать **отдельный Google Cloud проект**, выпустить **Service Account JSON-ключ** и **расшарить папку Google Drive** на этот SA. Результат — файл `gcp-sa.json`, который мы безопасно перенесём на VPS1 (мимо чата с AI).
