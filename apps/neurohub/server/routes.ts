...
export async function registerRoutes(
  ...
  // Admin: lockscreen-diag HTML страница (Eugene 2026-05-10).
  // Открой /api/admin/lockscreen-diag — вся диагностика обложек/ID3
  // по 30 последним music-трекам в виде таблицы. Скопируй и пришли.
  app.get("/api/admin/lockscreen-diag", (req: Request, res: Response) => {
    try {
      const raw = db.$client;
      const tracks = raw.prepare(`
        SELECT id, type, status, local_path as localPath, display_title as displayTitle,
               prompt, author_name as authorName, result_url as resultUrl, cover_gen_id as coverGenId
        FROM generations WHERE type = 'music' AND status = 'done' AND local_path IS NOT NULL
        ORDER BY id DESC LIMIT 30
      `).all() as any[];

      const rows = tracks.map((gen: any) => {
        const r: any = { id: gen.id, title: gen.displayTitle || (gen.prompt || "").slice(0, 40) };
        try {
          const mp3Path = path.join(AUTHORS_DIR, gen.localPath);
          r.mp3 = fs.existsSync(mp3Path);
          r.mp3KB = r.mp3 ? Math.round(fs.statSync(mp3Path).size / 1024) : 0;
          const taggedPath = mp3Path.replace(/\.mp3$/, ".tagged.mp3");
          r.tagged = fs.existsSync(taggedPath);
          r.taggedKB = r.tagged ? Math.round(fs.statSync(taggedPath).size / 1024) : 0;
          if (r.tagged) {
            try {
              const tags = NodeID3.read(taggedPath);
              r.id3Img = !!(tags && (tags as any).image);
              r.id3ImgKB = r.id3Img && (tags as any).image && (tags as any).image.imageBuffer
                ? Math.round((tags as any).image.imageBuffer.length / 1024) : 0;
            } catch (e) { r.id3Err = String(e).slice(0, 50); }
          }
          const cv = resolveCoverPath(gen);
          r.cover = !!cv && fs.existsSync(cv);
        } catch (e) { r.err = String(e).slice(0, 80); }
        return r;
      });

      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Lockscreen Diag</title>
<style>
body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#09090b;color:#e5e7eb;margin:20px;}
h1{color:#a78bfa;}table{border-collapse:collapse;width:100%;font-size:12px;}
th,td{border:1px solid #333;padding:6px 8px;text-align:left;}th{background:#1f1f23;}
.ok{color:#10b981;font-weight:bold;}.bad{color:#ef4444;font-weight:bold;}
.copy{background:#7c3aed;color:#fff;padding:8px 16px;border:none;border-radius:8px;cursor:pointer;margin-bottom:10px;}
pre{background:#1f1f23;padding:10px;border-radius:6px;overflow-x:auto;font-size:11px;}
.legend{margin:10px 0;font-size:12px;color:#888;}
</style></head><body>
<h1>🔍 Lockscreen Diag — ID3 embed status</h1>
<button class="copy" onclick="navigator.clipboard.writeText(document.getElementById('json').textContent).then(()=>{this.textContent='✅ Скопировано'})">📋 Скопировать JSON</button>
<div class="legend">
mp3 — есть ли исходный mp3 файл · tagged — создан ли tagged.mp3 (ID3-обёрнутая версия) ·
id3Img — встроена ли обложка в ID3 · cover — найден ли исходный cover файл
</div>
<table>
<thead><tr><th>id</th><th>title</th><th>mp3</th><th>KB</th><th>tagged</th><th>KB</th><th>id3Img</th><th>img KB</th><th>cover</th><th>err</th></tr></thead>
<tbody>${rows.map(r => `<tr>
<td>${r.id}</td>
<td>${(r.title || "").replace(/[<>&]/g,"")}</td>
<td class="${r.mp3?'ok':'bad'}">${r.mp3?'✓':'✗'}</td>
<td>${r.mp3KB || ''}</td>
<td class="${r.tagged?'ok':'bad'}">${r.tagged?'✓':'✗'}</td>
<td>${r.taggedKB || ''}</td>
<td class="${r.id3Img?'ok':'bad'}">${r.id3Img?'✓':r.tagged?'✗':'-'}</td>
<td>${r.id3ImgKB || ''}</td>
<td class="${r.cover?'ok':'bad'}">${r.cover?'✓':'✗'}</td>
<td>${r.err || r.id3Err || ''}</td>
</tr>`).join('')}</tbody>
</table>
<h3>JSON (для копирования)</h3>
<pre id="json">${JSON.stringify(rows, null, 2)}</pre>
</body></html>`;
