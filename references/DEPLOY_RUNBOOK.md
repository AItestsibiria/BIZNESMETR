...
## 0a. Если ты деплоишь с iPhone (никакого ноутбука)

Деплой полностью с телефона **возможен**. Установи два приложения и
используй Safari — этого хватит на все 12 шагов.

| Приложение | Цена | Зачем |
|---|---|---|
| **Termius** (App Store) | бесплатно | SSH-клиент. Заходить на сервер, редактировать `.env` и `secrets/...` через встроенный SFTP. |
| **Working Copy** (App Store) | бесплатно для read-only | Не обязательно, но удобно — смотреть код с телефона. Push через GitHub Mobile тоже работает. |

Особенности iPhone-пути:

- **Шаг 1** (создать VPS на Timeweb) делается через `timeweb.cloud` в Safari
  — мобильная вёрстка нормальная, 5 минут кликов.
- **Шаги 2–10** (SSH-команды) запускаются из Termius — копи-пейст из этого
  runbook'а работает.
- **Шаг 5** (положить `google-credentials.json` на сервер) — в Termius
  переключись в SFTP-режим, загрузи файл с iCloud Drive или Files.app.
  Альтернатива: открой файл в Files.app, скопируй содержимое, на сервере
  `nano secrets/google-credentials.json` → вставить → Ctrl+O → Ctrl+X.
- **Шаг 6** (заполнить `.env`) — то же самое через `nano` в Termius.
- **Шаг 9** (зарегистрировать Telegram webhook) — `curl` команда прямо из
  Termius, ничего не меняется.
- **После шага 11** — авто-деплой настроен → **iPhone больше не нужен для
  деплоя**. Ты пишешь мне в чат, я пушу в `develop`, GitHub Actions сам
  выкатывает на сервер. Сервер ты трогаешь только при правке `.env`.

> **Совет:** ssh-ключ для шага 1 сгенери прямо в Termius (Settings → Keychain → New key → ED25519), его публичную часть Termius даст скопировать. Эту строку и вставляешь в Timeweb при создании сервера. Так у тебя один ключ управляет всем процессом и не нужно ничего переносить между устройствами.

---

## 0b. (Опционально) Поднять VPS через API одной командой

Если ты с ноута / desktop'а и хочешь автоматизировать создание сервера —
скрипт `scripts/provision-timeweb.sh` делает всё за один прогон:
валидирует токен, находит подходящий конфигуратор и образ Ubuntu 22.04,
создаёт сервер с нужными CPU/RAM/диском, ждёт IP.

```bash
# на твоей машине (не на сервере):
export TIMEWEB_TOKEN='<токен из Timeweb → API; никогда не пиши его в чат>'
bash scripts/provision-timeweb.sh
```

Опциональные переменные перед запуском: `NAME`, `CPU`, `RAM_MB`,
`DISK_GB`, `LOCATION`, `OS_NAME`, `SSH_KEY_IDS`. По умолчанию — наша
экономная конфигурация (1 vCPU / 2 GB / 30 GB / Ubuntu 22.04 / ru-1).

Скрипт **никогда не эхо-ит токен** в stdout / лог. Сразу после успешного
прогона — revoke токен в панели и продолжай с шага 2 runbook'а.

С iPhone этот путь не работает (bash + jq на телефоне нет). Используй
секцию 0a (через web UI Timeweb).
