...
// ярче и крупнее звезда: Сириус (-1.46) ≈ 5px, mag 1 ≈ 3px, mag 3 ≈ 1.5px.
export function magToSize(mag: number): number {
  // линейная аппроксимация: size = clamp(5 - 0.85 * mag, 1.0, 6.0)
  return Math.max(1.0, Math.min(6.0, 5.0 - 0.85 * mag));
}

// Magnitude → opacity (0.4..1.0): тусклые звёзды чуть прозрачнее.
export function magToOpacity(mag: number): number {
  return Math.max(0.45, Math.min(1.0, 1.1 - 0.13 * mag));
}
...
export function spectralToColor(cls: SpectralClass): [number, number, number] {
  switch (cls) {
    case "O": return [0.61, 0.71, 1.00]; // голубой
    case "B": return [0.78, 0.85, 1.00]; // бело-голубой
    case "A": return [0.96, 0.97, 1.00]; // белый
    case "F": return [1.00, 0.98, 0.92]; // бледно-жёлтый
    case "G": return [1.00, 0.95, 0.78]; // жёлтый
    case "K": return [1.00, 0.83, 0.62]; // оранжевый
    case "M": return [1.00, 0.68, 0.50]; // красный
