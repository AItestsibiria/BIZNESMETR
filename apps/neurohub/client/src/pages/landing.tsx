...
// Eugene 2026-05-30 (Босс «кнопки в едином стиле») — единые back/share
// (lucide-иконки + adaptive sizing + brand-цвета). CLAUDE.md:
...
function PlaylistSection({ autoPlayId }: { autoPlayId?: number }) {
  ...
  // Eugene 2026-05-30: глобальный listener `muza:close-globe` для
  // <BackToMuzaButton/> (Unified-back-share-buttons rule, дефолтное действие).
  // Также используется компонентами вне 3D-card, которым нужно закрыть глобус.
  useEffect(() => {
    const onClose = () => {
      if (showGlobe) {
        setShowGlobe(false);
    ...
    window.addEventListener("muza:close-globe", onClose);
    return () => window.removeEventListener("muza:close-globe", onClose);
  }, [showGlobe]);
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              {/* ScreenDebugOverlay mount уже в App.tsx (глобально) */}
                      ...
                      {showGlobe && globe3dEnabled && createPortal(
                        ...
                        >
                          ...
                          <button
                            ...
                            className="fixed right-2 bottom-2 z-[260] w-14 h-14 rounded-full flex items-center justify-center text-base bg-purple-500/10 border border-purple-300/20 active:bg-purple-500/40 active:scale-90 transition-all"
